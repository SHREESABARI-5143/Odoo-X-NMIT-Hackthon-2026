...
async function getEmployees(req, res) {
    try {
        ...
        const todayAttendances = await prisma_1.prisma.attendance.findMany({
            where: { date: todayStr },
        });
...
async function getEmployeeById(req, res) {
    try {
        ...
        const plainBankAccount = (0, crypto_1.decrypt)(user.bankAccount) || user.bankAccount;
        const plainIfsc = (0, crypto_1.decrypt)(user.ifsc) || user.ifsc;
        const plainPan = (0, crypto_1.decrypt)(user.pan) || user.pan;
        const plainUan = (0, crypto_1.decrypt)(user.uan) || user.uan;
        const plainPersonalEmail = (0, crypto_1.decrypt)(user.personalEmail) || user.personalEmail;
        const plainAddress = (0, crypto_1.decrypt)(user.address) || user.address;
        const maskedAccount = (0, crypto_1.maskBankAccount)(user.bankAccount);
        if (!isAdmin && !isSelf) {
...
async function createEmployee(req, res) {
    try {
        const { companyName, firstName, lastName, email, phone, password, jobPosition, department, manager, location, role, dateOfJoining, } = req.body;
        ...
        const loginId = await (0, loginIdGenerator_1.generateLoginId)(cName, firstName, lastName, dateOfJoining);
        ...
        const newUser = await prisma_1.prisma.user.create({
            data: {
                ...
                bankAccount: (0, crypto_1.encrypt)('123456789012'),
                bankName: 'HDFC Bank',
                ifsc: (0, crypto_1.encrypt)('HDFC0001234'),
                pan: (0, crypto_1.encrypt)('ABCDE1234F'),
                uan: (0, crypto_1.encrypt)('100900800700'),
        ...
        await prisma_1.prisma.leaveAllocation.create({
            data: {
        ...
        await prisma_1.prisma.salaryConfig.create({
            data: {
                employeeId: newUser.id,
                monthlyWage: 60000,
                yearlyWage: 720000,
...
async function updateEmployee(req, res) {
    try {
        ...
        const { firstName, lastName, phone, address, dob, nationality, personalEmail, gender, maritalStatus, about, interests, avatarUrl, jobPosition, department, manager, location, } = req.body;
        const dataToUpdate = {};
        if (phone !== undefined)
            dataToUpdate.phone = phone;
        if (address !== undefined)
            dataToUpdate.address = (0, crypto_1.encrypt)(address);
        if (dob !== undefined)
            dataToUpdate.dob = dob;
        if (nationality !== undefined)
            dataToUpdate.nationality = nationality;
        if (personalEmail !== undefined)
            dataToUpdate.personalEmail = (0, crypto_1.encrypt)(personalEmail);
        if (gender !== undefined)
            dataToUpdate.gender = gender;
        if (maritalStatus !== undefined)
            dataToUpdate.maritalStatus = maritalStatus;
        if (about !== undefined)
        ...
        if (currentUserRole === 'ADMIN_HR') {
            ...
            if (jobPosition !== undefined)
                dataToUpdate.jobPosition = jobPosition;
            if (department !== undefined)
                dataToUpdate.department = department;
            if (manager !== undefined)
                dataToUpdate.manager = manager;
...
async function addCertification(req, res) {
    try {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found.' });
        const existingCerts = user.certifications ? JSON.parse(user.certifications) : [];
        if (name && issuer) {
